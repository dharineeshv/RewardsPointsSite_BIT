...
export default async function handler(req, res) {
  ...
  try {
    ...
    const activeToken = token || process.env.PS_SERVICE_TOKEN;
    if (activeToken) {
      try {
...
function formatAttendanceData(rawJson, targetDate, rollNo) {
  const defaultTimings = [
    ...
    { id: 3, timing: '10:45 am to 11:35 am', slot: 'Forenoon', section: 'Forenoon' },
    { id: 4, timing: '11:35 am to 12:25 pm', slot: 'Forenoon', section: 'Forenoon' },
    { id: 5, timing: '01:25 pm to 02:15 pm', slot: 'Afternoon', section: 'Afternoon' },
    { id: 6, timing: '02:15 pm to 03:05 pm', slot: 'Afternoon', section: 'Afternoon' },
    { id: 7, timing: '03:15 pm to 04:30 pm', slot: 'Afternoon', section: 'Afternoon' }
  ];

  const rawPeriods = rawJson.periods || rawJson.data || rawJson.attendance || [];
  const sessions = defaultTimings.map((slot, idx) => {
    const p = rawPeriods[idx] || {};
    return {
      id: slot.id,
      timing: p.time || p.timing || slot.timing,
      slot: slot.slot,
      section: slot.section,
      markedBy: p.faculty || p.marked_by || '—',
      status: p.status || (p.is_present ? 'Present' : 'Absent')
    };
  });

  const presentCount = sessions.filter(s => s.status === 'Present').length;
  const totalWorkingDays = rawJson.total_working_days || 71;
  const daysPresent = rawJson.days_present !== undefined ? rawJson.days_present : (presentCount > 0 ? 71 : 70);
  const daysAbsent = totalWorkingDays - daysPresent;
  const percentage = rawJson.percentage || ((daysPresent / totalWorkingDays) * 100).toFixed(2);
